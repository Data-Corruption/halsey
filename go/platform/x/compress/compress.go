...
func InitHWAccel(ctx context.Context) {
	ActiveHWAccel = detectHWAccel()
	xlog.Infof(ctx, "hardware accel detected: %s", ActiveHWAccel.String())

	args := []string{}

	switch ActiveHWAccel {
	case HWAccelNvidia:
		args = append(args,
			"-hwaccel", "cuda",
			"-c:v", "hevc_nvenc",
